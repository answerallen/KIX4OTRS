...
sub TableCreateComplex {
    ...
    my @HeadLine;
    my @Columns
        = ( 'ChangeStateSignal', 'ChangeNumber', 'ChangeTitle', 'ChangeState', 'ChangeTime' );
    if (
        defined $Param{EnabledColumns}->{'ITSMChange'}
        && scalar @{ $Param{EnabledColumns}->{'ITSMChange'} }
        )
    {
        @Columns = @{ $Param{EnabledColumns}->{'ITSMChange'} };
    }
        ...
        )
    {
        ...
        for my $Column (@Columns) {
            ...
            $TempHash{Content} = $Change->{$Column};
            $TempHash{Key}     = $ChangeID;

            # get type
            if ( $Column =~ /ChangeStateSignal/ ) {
                $TempHash{Type}        = 'ChangeStateSignal';
                $TempHash{ChangeState} = $Change->{$Column};
            }
            elsif ( $Column =~ /ChangeNumber/ ) {
                $TempHash{Type} = 'Link';
                $TempHash{Link}
                    = $Kernel::OM->Get('Kernel::Output::HTML::Layout')->{Baselink} . 'Action=AgentITSMChangeZoom;ChangeID=' . $ChangeID;
            }
            elsif ( $Column =~ /Time/ ) {
        ...
        #         Link    => $Kernel::OM->Get('Kernel::Output::HTML::Layout')->{Baselink} . 'Action=AgentITSMChangeZoom;ChangeID=' . $ChangeID,
        #     },
        #     {
        #         Type      => 'Text',
