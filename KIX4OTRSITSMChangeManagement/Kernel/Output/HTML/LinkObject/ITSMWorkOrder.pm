...
sub TableCreateComplex {
    ...
    my @Columns
        = (
        'WorkOrderStateSignal', 'ChangeNumber', 'WorkOrderTitle', 'ChangeTitle',
        'WorkOrderState',       'ChangeTime'
        );
    if (
        defined $Param{EnabledColumns}->{'ITSMWorkOrder'}
        && scalar @{ $Param{EnabledColumns}->{'ITSMWorkOrder'} }
        )
    {
        @Columns = @{ $Param{EnabledColumns}->{'ITSMWorkOrder'} };
    }
    ...
    for my $Column (@Columns) {
        ...
        elsif ( $Column =~ /ChangeNumber/ ) {
            $TempHash{Content} = $Self->{ChangeHook};
        }
        else {
            $TempHash{Content} = $Column;
        }
        push @HeadLine, \%TempHash;
    }
    # EO KIX4OTRS-capeIT
        ...
        )
    {
        ...
        for my $Column (@Columns) {
            ...
            # get content
            if ( $Column =~ /Change/ ) {
                $TempHash{Content} = $WorkOrder->{ChangeData}->{$Column};
            }
            else {
                $TempHash{Content} = $WorkOrder->{$Column};
            }

            # get key
            $TempHash{Key} = $WorkOrderID;

            # get type
            if ( $Column =~ /WorkOrderStateSignal/ ) {
                $TempHash{Type}        = 'WorkOrderStateSignal';
                $TempHash{ChangeState} = $WorkOrder->{$Column};
            }
            elsif ( $Column =~ /ChangeNumber/ ) {
                $TempHash{Type}    = 'Link';
                $TempHash{Content} = $WorkOrder->{ChangeData}->{ChangeNumber}
                    . '-' . $WorkOrder->{WorkOrderNumber};
                $TempHash{Link}
                    = $Kernel::OM->Get('Kernel::Output::HTML::Layout')->{Baselink} . 'Action=AgentITSMWorkOrderZoom;WorkOrderID=' . $WorkOrderID;
            }
            elsif ( $Column =~ /Time/ ) {
                $TempHash{Type} = 'TimeLong';
            }
            else {
                $TempHash{Type} = 'Text';
            }
            ...
            if ( $Column =~ /State/ ) {
                $TempHash{Translate} = 1;
            }
            if ( $Column =~ /Title/ ) {
                $TempHash{MaxLength} = 70;
            }

            push @ItemColumns, \%TempHash;
        }
      ...
      #             . '-' . $WorkOrder->{WorkOrderNumber},
      #         Link => $Kernel::OM->Get('Kernel::Output::HTML::Layout')->{Baselink} . 'Action=AgentITSMWorkOrderZoom;WorkOrderID=' . $WorkOrderID,
      #     },
      #     {
      #         Type      => 'Text',
